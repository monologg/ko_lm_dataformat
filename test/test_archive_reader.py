...
def test_dat():
    archive = klmd.DatArchive("test_dir")
...
def test_json():
    archive = klmd.JSONArchive("test_dir")
    blns = open("test/blns.txt").read()
    archive.add_data(blns)
    archive.add_data("testing 123")
    archive.add_data(blns)
...
def test_jsonl_tar():
    blns = open("test/blns.txt").read()
    reader = klmd.Reader("test/blns.jsonl.zst.tar")
...
def test_txt_read():
    reader = klmd.Reader("test/blns.txt")
    blns = open("test/blns.txt").read()
...
def test_zip_read():
    reader = klmd.Reader("test/blns.txt.zip")
    blns = open("test/blns.txt").read()
...
def test_tgz_read():
    reader = klmd.Reader("test/blns.txt.tar.gz")
    blns = open("test/blns.txt").read()
...
def test_tarfile_reader():
    rdr = klmd.tarfile_reader(open("test/testtarfile.tar", "rb"), streaming=True)
